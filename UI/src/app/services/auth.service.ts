...
export class AuthService {
  ...
  constructor() {
    ...
    for (const eventName of ["click", "input", "change", "submit", "keydown"]) {
      document.addEventListener(eventName, event => {
        ...
        if (eventName === "click" || eventName === "keydown") {
          if (!el?.closest?.("button, a, input, select, textarea, [role=button], [contenteditable=true]")) return;
        }
        this.recordActivity();
  ...
  private checkIdle(): boolean {
    // Idle logout is driven only by continuous inactivity (5 minutes).
    // JWT absolute expiry is enforced by the APIs; do not log active users out
    // solely because the original token issue time is approaching.
